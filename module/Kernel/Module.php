...
class Module implements AutoloaderProviderInterface {
	...
	public function getServiceConfig() {
		...
		$dir = DOC_ROOT . "/module/" . __NAMESPACE__ . "/Db/Generated";
		if (is_dir ( $dir )) {
			...
			foreach ( $dir as $dirInfo ) {
				if (! $dirInfo->isDot ()) {
					...
					if (! strstr ( $ClassName, "Table" ) && ! strstr ( $ClassName, "Finder" )) {
						$path = __NAMESPACE__ . "\Db\Generated\\";
						...
						$factory [$gateway] = function ($sm) use($name, $entity, $class, $path) {
							...
							return new TableGateway ( strtolower ( preg_replace ( '/(?<!^)([A-Z])/', '_\\1', $name ) ), $dbAdapter, null, $initResultSet );
						};
					}
				}
			}
